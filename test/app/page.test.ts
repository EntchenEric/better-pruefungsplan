...
jest.mock('@//components/ExamScheduleViewer', () => () =>
  React.createElement('div', { 'data-testid': 'exam-schedule-viewer' }, 'Mocked ExamScheduleViewer'),
);
jest.mock('../../src/components/ExamScheduleViewer', () => () =>
  React.createElement('div', { 'data-testid': 'exam-schedule-viewer' }, 'Mocked ExamScheduleViewer'),
);
...
jest.mock('react-icons/bs', () => ({
  ...
  BsLightbulb: () => React.createElement('svg', { 'data-testid': 'lightbulb-icon' }),
}));
jest.mock('react-icons/io5', () => ({
  IoWarning: () => React.createElement('svg', { 'data-testid': 'warning-icon' }),
}));
...
describe('Home page', () => {
  ...
  it('renders GitHub repository link with correct attributes and icon', () => {
    render(React.createElement(Home));
    const repoLink = screen.getByText('https://github.com/EntchenEric/better-pruefungsplan') as HTMLAnchorElement;
    expect(repoLink).toBeTruthy();
    expect(repoLink.tagName.toLowerCase()).toBe('a');
    expect(repoLink.getAttribute('href')).toBe('https://github.com/EntchenEric/better-pruefungsplan');
  ...
  it('renders issue and feedback links with attributes and icons', () => {
    render(React.createElement(Home));
    const bugLink = screen.getByRole('link', { name: /bug melden/i }) as HTMLAnchorElement;
    expect(bugLink.getAttribute('href')).toBe('https://github.com/EntchenEric/better-pruefungsplan/issues/new');
    ...
    const feedbackLink = screen.getByRole('link', { name: /feedback & ideen teilen/i }) as HTMLAnchorElement;
    expect(feedbackLink.getAttribute('href')).toBe('https://github.com/EntchenEric/better-pruefungsplan/discussions');
    expect(feedbackLink.getAttribute('target')).toBe('_blank');
    expect(feedbackLink.getAttribute('rel')).toBe('noopener noreferrer');
    expect(feedbackLink.getAttribute('title')).toBe('Feedback oder Ideen einreichen');
  ...
  it('renders the warning block and explanatory text', () => {
    ...
    expect(screen.getByText(/Die Informationen auf dieser Seite können falsch sein/)).toBeTruthy();
  ...
  it('has the responsive flex container with alignment classes', () => {
    const { container } = render(React.createElement(Home));
    const flex = container.querySelector('div.flex.flex-col[class*="md:flex-row"]') as HTMLElement | null;
