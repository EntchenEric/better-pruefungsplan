...
jest.mock('@//components/StickyHeader', () => {
  return {
    StickyHeader: ({ onToggleColumn, onGlobalSearchChange, setSelectedCourse, setSelectedSemester }: any) => (
      <div data-testid="sticky-header">
        <button type="button" onClick={() => onToggleColumn('testCol')} data-testid="toggle-column">
          Toggle Column
        </button>
        <input 
        ...
        />
        <button type="button" onClick={() => setSelectedCourse('TEST101')} data-testid="set-course">
          Set Course
        </button>
        <button type="button" onClick={() => setSelectedSemester('WS2024')} data-testid="set-semester">
          Set Semester
        </button>
      </div>
    ),
  ...
  return {
    ExamTableHeader: ({ onColumnFilterChange, setColWidths }: any) => (
      <thead data-testid="exam-table-header">
        <tr>
          <th>
            <button type="button" onClick={() => onColumnFilterChange('course', 'test')} data-testid="filter-column">
              Filter
            </button>
            <button type="button" onClick={() => setColWidths('course', 100)} data-testid="resize-column">
              Resize
            </button>
          </th>
        </tr>
...
const mockUseExamFiltering = useExamFiltering as jest.MockedFunction<typeof useExamFiltering>;
...
describe('ExamScheduleViewer', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders the table with correct accessibility attributes', () => {
      ...
      expect(table).toHaveClass('w-full', 'border-collapse', 'table-fixed', 'user-select-none', 'select-none');
    });
    ...
    it('displays the correct entry count', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Gefundene Einträge: 1 / 2')).toBeInTheDocument();
      });
    });
    ...
    it('renders with correct CSS classes and structure', () => {
      ...
      const container = screen.getByText('Gefundene Einträge: 1 / 2').closest('div');
      expect(container).toHaveClass('mt-5', 'text-center', 'text-secondary-text', 'italic', 'text-base', 'select-none');
      
      const tableContainer = screen.getByRole('table').closest('div');
      expect(tableContainer).toHaveClass('overflow-x-auto', 'rounded-lg', 'shadow-md', 'border', 'border-secondary-text');
    });
  });
  ...
  describe('Data Fetching', () => {
    ...
    it('handles fetch response errors', async () => {
      ...
      await waitFor(() => {
        expect(mockConsoleError).toHaveBeenCalledWith('Error parsing PDF:', expect.any(Error));
      });
    });
    ...
    it('handles network errors during fetch', async () => {
      ...
      await waitFor(() => {
        expect(mockConsoleError).toHaveBeenCalledWith('Error parsing PDF:', networkError);
      });
    });
  ...
  describe('Event Handlers', () => {
    ...
    it('calls handleGlobalSearchChange when triggered', () => {
      ...
      fireEvent.change(screen.getByTestId('global-search'), { target: { value: 'test search' } });
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('handles mismatched entries and filtered entries counts', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Gefundene Einträge: 0 / 2')).toBeInTheDocument();
      });
    });
    ...
    it('handles fetch timeout scenarios', async () => {
      ...
      await waitFor(() => {
        expect(mockConsoleError).toHaveBeenCalledWith('Error parsing PDF:', timeoutError);
      });
    });
  });
