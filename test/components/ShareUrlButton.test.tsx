...
describe('ShareUrlButton', () => {
  ...
  const restoreClipboard = () => {
    if (originalClipboard) {
      ...
      Object.defineProperty(navigator as any, 'clipboard', { value: undefined, configurable: true });
    }
  };
  ...
  it('renders button with icon, label and expected classes', () => {
    ...
    const button = screen.getByRole('button', { name: /share current url with filters/i });
  ...
  describe('Clipboard API - success path', () => {
    ...
    it('copies current URL using navigator.clipboard.writeText', () => {
      ...
      expect(navigator.clipboard!.writeText).toHaveBeenCalledWith('https://example.com/test-page');
    });
  ...
  describe('Clipboard API - rejection triggers fallback', () => {
    const setupDomFallbackSpies = () => {
      ...
      const createElement = jest
        ...
        .mockImplementation((tagName: string): any => {
          if (tagName.toLowerCase() === 'textarea') return textArea as HTMLTextAreaElement;
    ...
    it('falls back to document.execCommand("copy") on writeText failure', async () => {
      const { textArea, createElement, appendChild, removeChild, execCommand } = setupDomFallbackSpies();
  ...
  describe('URL variations and edge cases', () => {
    ...
    it('copies URL with query parameters', () => {
      ...
      expect(navigator.clipboard!.writeText).toHaveBeenCalledWith('https://example.com/page?param1=a&param2=b');
    });
    ...
    it('copies URL with hash fragment', () => {
      ...
      expect(navigator.clipboard!.writeText).toHaveBeenCalledWith('https://example.com/page#section1');
    });
  ...
  describe('Multiple rapid clicks', () => {
    ...
    it('invokes clipboard writeText on each click and resets toast timer', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('URL in Zwischenablage kopiert!')).toBeFalsy();
      });
    });
  });
});