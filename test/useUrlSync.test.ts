...
const mockDecodeColumnVisibility = decodeColumnVisibility as unknown as jest.Mock;
...
beforeEach(() => {
  ...
  mockIsSemester.mockImplementation((v: string) => ['WS23', 'SS24'].includes(v));
...
describe('useUrlSync - debounced URL updates', () => {
  test('debounces updates and calls router.replace with query string when params not empty', () => {
    ...
    mockCreateSearchParams.mockImplementation(() => new URLSearchParams('q=1&x=2'));
    ...
    expect(replaceMock).toHaveBeenCalledWith('/exam?q=1&x=2', { scroll: false });
  });
  ...
  test('only last change within debounce window triggers one replace', () => {
    ...
    mockCreateSearchParams.mockImplementation(
      (s: string) => new URLSearchParams(s ? `search=${encodeURIComponent(s)}` : '')
    );
    ...
    expect(replaceMock).toHaveBeenCalledTimes(1);
    expect(replaceMock).toHaveBeenCalledWith('/exam?search=ab', { scroll: false });
  });
