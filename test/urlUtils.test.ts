...
describe('encodeColumnFilters', () => {
  ...
  test('produces expected base64 for a simple, stable object (single key)', () => {
    ...
    expect(encodeColumnFilters(valueToTest)).toBe(btoa(JSON.stringify(valueToTest)));
  });
...
describe('decodeColumnFilters', () => {
  const defaultFilters = Object.fromEntries(TABLE_HEADERS.map(h => [h.key, '']));
  ...
  test('atob throwing is caught and returns defaults', () => {
    ...
    global.atob = () => { throw new Error('bad base64'); };
    const spy = jest.spyOn(console, 'warn').mockImplementation(() => {});
...
describe('encodeColumnVisibility', () => {
  ...
  test('encodes only changed visibilities', () => {
    ...
    v[firstHidden] = false;
    const firstVisible = TABLE_HEADERS.find(h => !DEFAULT_HIDDEN_COLUMNS.includes(h.key))!.key;
    v[firstVisible] = true;
...
describe('decodeColumnVisibility', () => {
  ...
  test('merges decoded values into defaults', () => {
    ...
    const encoded = btoa(JSON.stringify(partial));
    expect(decodeColumnVisibility(encoded)).toEqual({ ...defaultVis, ...partial });
  });
...
describe('encodeColumnWidths', () => {
  ...
  test('encodes only changed widths', () => {
    const modified = { ...DEFAULT_COLUMN_WIDTHS, po: DEFAULT_COLUMN_WIDTHS.po + 10, mid: DEFAULT_COLUMN_WIDTHS.mid - 5 };
    const res = encodeColumnWidths(modified as any);
    const obj = JSON.parse(atob(res));
    expect(obj).toEqual({ po: DEFAULT_COLUMN_WIDTHS.po + 10, mid: DEFAULT_COLUMN_WIDTHS.mid - 5 });
  });

  test('does not mutate input', () => {
    const modified = { ...DEFAULT_COLUMN_WIDTHS, po: DEFAULT_COLUMN_WIDTHS.po + 10 };
...
describe('createSearchParams', () => {
  test('includes only non-empty, valid params', () => {
    ...
    const widths = { ...DEFAULT_COLUMN_WIDTHS, po: DEFAULT_COLUMN_WIDTHS.po + 5 };
    const course = 'AI';
    const semester = 'SS24';

    const params = createSearchParams(globalSearch, columnFilters as any, visibility as any, widths as any, course, semester);
    ...
    expect(JSON.parse(atob(widthsParam!))).toEqual({ po: DEFAULT_COLUMN_WIDTHS.po + 5 });
  ...
  test('works when some inputs are undefined for course/semester', () => {
    const params = createSearchParams('', {} as any, {} as any, DEFAULT_COLUMN_WIDTHS as any, undefined, undefined);
    expect(params.get('course')).toBeNull();
    expect(params.get('semester')).toBeNull();
  });
  ...
  test('end-to-end: params reflect encode functions results', () => {
    ...
    vis.mid = true;
    const widths = { ...DEFAULT_COLUMN_WIDTHS, datum: DEFAULT_COLUMN_WIDTHS.datum + 33 };

    const params = createSearchParams('x', filters as any, vis as any, widths as any, 'AI', 'WS23');
