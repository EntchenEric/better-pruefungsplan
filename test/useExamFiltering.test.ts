...
describe('useExamFiltering', () => {
  ...
  test('global search matches any field, case-insensitive; does not trim the search string for matching', () => {
    ...
    let { result, rerender } = renderHook(
      ({ search }) => useExamFiltering(ENTRIES as any, search, {}, undefined, undefined),
    ...
    expect(result.current.map(e => e.modul)).toEqual(['Software Engineering']);

    // Uppercase still matches (case-insensitive)
    rerender({ search: 'DR.' });
    expect(result.current.map(e => e.pruefer)).toEqual(['Dr. Alice', 'Dr. Bob', 'Dr. Eve']);
  });

  test('columnFilters: single key, includes, case-insensitive, trims', () => {
    const filters = { modul: '  math ' };
    const { result } = renderHook(() =>
      useExamFiltering(ENTRIES as any, '', filters as any, undefined, undefined)
    );
  ...
  test('selectedCourse only: filters to entries with non-empty value in that course column', () => {
    ...
    expect(result.current.map(e => e.modul)).toEqual(['Mathematics', 'algorithms']);
  });
  ...
  test('rerender: responds to prop changes (memo dependencies)', () => {
    const { result, rerender } = renderHook(
      ({ search, filters, course, semester }) =>
        useExamFiltering(ENTRIES as any, search, filters as any, course, semester),
      {
        initialProps: {
          search: '',
          filters: {},
    ...
    rerender({ search: 'bob', filters: {}, course: undefined, semester: undefined });
    expect(result.current.map(e => e.pruefer)).toEqual(['Dr. Bob']);

    rerender({ search: '', filters: { po: '2016' }, course: undefined, semester: undefined });
    expect(result.current.map(e => e.po)).toEqual(['2016', '2016', '2016']);

    rerender({ search: '', filters: {}, course: 'SE', semester: undefined });
    expect(result.current.map(e => e.modul)).toEqual(['Software Engineering', 'algorithms']);
